...
$('#prevtab').on(
		'click',
		function() {
			$tabs.filter('.active').prev('li').find('a[data-toggle="tab"]')
					.tab('show');
...
$('#nexttab').on(
		'click',
		function() {
			$tabs.filter('.active').next('li').find('a[data-toggle="tab"]')
					.tab('show');
...
function revisao() {
	...
	$(".campo").each(
			function(index) {
				...
				$("#Lista-pontos-ajax").append(
						'<div class="lista-pontos"><strong>' + $(this).val()
								+ '</strong><br>' + valorTextArea + '</div>');
			});
}